...
<section class="reviews-container">
   <div class="heading">
      ...
      <a href="add_review.php?id=<?= $postId ?>&title=<?= urlencode($title) ?>&overview=<?= urlencode($overview) ?>&image=<?= urlencode($image) ?>&vote_average=<?= $voteAverage ?>" class="inline-option-btn" style="margin-top: 0;">add review</a>
   </div>
            ...
            <div class="box" <?php if ($fetch_review['user_id'] == $user_id) {
               ...
               <?php if ($fetch_review['user_id'] == $user_id) { ?>
                  <form action="" method="post" class="flex-btn" style="justify-content: flex-end;">
                     ...
                     <input type="submit" value="delete review" class="know-more" name="delete_review" onclick="return confirm('delete this review?');">
                  </form>
               <?php }; ?>
            </div>
      <?php
