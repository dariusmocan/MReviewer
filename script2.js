...
function openPost(movie) {
    const { id, title, overview, poster_path, vote_average } = movie;

    // Construct the URL for the view_post.php file with the necessary parameters
    const url = `view_post.php?id=${id}&title=${encodeURIComponent(title)}&overview=${encodeURIComponent(overview)}&image=${encodeURIComponent(IMG_URL + poster_path)}&vote_average=${vote_average}`;
